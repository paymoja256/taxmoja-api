...
class InvoiceHandler:
    ...
    def create_outgoing_invoice(
        self, db, tax_invoice: TaxInvoiceIncomingSchema, country_code, tax_id
    ):
        message, new_invoice = create_outgoing_invoice(
            db, tax_invoice, country_code, tax_id
        )

        struct_logger.info(
            event="create_outgoing_invoice",
            message=message,
    ...
    async def send_invoice(self, db, invoice: TaxInvoiceOutgoing):
        ...
        struct_logger.info(
            event="sending_incoming_invoice",
        ...
        if invoice.status == InvoiceStatuses.RECEIVED:
            try:
                request_invoice = invoice.request_invoice or parse_obj_as(
                    TaxInvoiceIncomingSchema, json.loads(invoice.request_data)
                )
    ...
    @staticmethod
    async def get_invoice_by_id(db, instance_invoice_id, country_code, tax_id):
    ...
    def generate_qr_code(self, qr_text, country_code, tax_pin, qr_name):
        ...
        qr_code.png(
            "app/api/static/qr_codes/{}/{}/{}.png".format(
                country_code, tax_pin, qr_name
            ),
            scale=2,
        )
