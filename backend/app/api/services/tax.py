...
from app.db.schemas.stock import (
    IncomingStockConfigurationSchema,
    IncomingGoodsStockAdjustmentSchema,
)
...
class TaxService:
    def __init__(self, interface_details: ClientAuthenticationBase, client_settings):
        ...
        self.settings = (
            ...
            else self.client["production"]
        )
        self.invoice_module = importlib.import_module(
            "app.api.handlers.%s.invoice_handler" % self.country_code
        )
        self.stock_module = importlib.import_module(
            "app.api.handlers.%s.stock_handler" % self.country_code
        )
        self.information_module = importlib.import_module(
            "app.api.handlers.%s.information_handler" % self.country_code
        )
        ...
        self.information_handler = getattr(
            self.information_module, "TaxInformationHandler"
        )
    ...
    def create_outgoing_invoice(self, db, tax_invoice: TaxInvoiceOutgoingSchema):
        ...
        new_invoice = self.invoice_manager.create_outgoing_invoice(
            db, tax_invoice, self.country_code, self.tax_id
        )
        return new_invoice
    ...
    async def get_invoice_by_id(self, db, instance_invoice_id):
        invoice = await self.invoice_manager.get_invoice_by_id(
            db, instance_invoice_id, self.country_code, self.tax_id
        )
        return invoice

    async def get_invoice_by_instance_id(self, db, instance_invoice_id):
        invoice = await self.invoice_manager.get_invoice_by_instance_id(
            db, instance_invoice_id, self.country_code, self.tax_id
        )
        return invoice

    async def get_invoice_query_by_instance_id(self, db, instance_invoice_id):
        invoice = await self.invoice_manager.get_invoice_by_instance_id_query(
            db, instance_invoice_id, self.country_code, self.tax_id
        )
        return invoice

    async def query_credit_notes(self):
        credit_notes = await self.invoice_manager.credit_note_query()
        return credit_notes
    ...
    async def send_stock_configuration(self, db, StockBase: Stock):
        new_stock_configuration = await self.stock_manager.send_stock_configuration(
            db, StockBase
        )
        return new_stock_configuration

    async def send_goods_stock_adjustment(
        self, db, stock_adjustment: IncomingGoodsStockAdjustmentSchema
    ):
        new_stock_adjustment = await self.stock_manager.send_goods_stock_adjustment(
            db, stock_adjustment
        )
